  ...
  useEffect(() => {
    async function fetchDeal() {
      ...
      if (!error && data) {
        ...
        // ✨ [Fix 7] 종료 판정: 3일 이상 된 딜은 종료 가능성 표시
        if (data.crawled_at) {
          const crawledAt = new Date(data.crawled_at);
          const hoursElapsed = (Date.now() - crawledAt.getTime()) / (1000 * 60 * 60);
  ...
  return (
    <div className="max-w-4xl mx-auto bg-gray-50 min-h-screen pb-10">
      <header className="bg-white border-b p-3 sticky top-0 z-10 shadow-sm flex items-center gap-3">
        ...
        <a href="https://www.ssagesage.com/" className="text-xl font-bold text-blue-800 truncate flex-1 hover:text-blue-600 transition-colors">
          싸게사게
        </a>
      </header>
      ...
      <main className="p-3 flex flex-col gap-3">
        ...
        {isExpired && (
          <div className="bg-yellow-50 border border-yellow-200 rounded-2xl px-4 py-3 flex items-center gap-2">
            ...
            <div>
              <p className="text-xs font-bold text-yellow-700">이 핫딜은 종료되었을 수 있습니다</p>
              <p className="text-[10px] text-yellow-600 mt-0.5">수집된 지 3일 이상 지난 딜입니다. 쇼핑몰에서 재고 및 가격을 꼭 확인해 주세요.</p>
            </div>
          </div>
        )}
