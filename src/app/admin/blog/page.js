...
export default function AdminBlogPage() {
  ...
  async function fetchData() {
    ...
    const [postsRes, catsRes] = await Promise.all([
      supabase
        .from('blog_posts')
        .select('id, slug, title, emoji, published, created_at, updated_at, category_id')
  ...
  async function addCategory() {
    ...
    const { data, error } = await supabase
      .from('blog_categories')
      .insert({ name: newCatName.trim(), slug, sort_order: maxOrder + 1 })
      .select()
      .single();
    setCatSaving(false);
  ...
  async function deleteCategory(cat) {
    ...
    if (postsInCat.length > 0) {
      alert(`이 카테고리에 글이 ${postsInCat.length}개 있어서 삭제할 수 없어요!\n먼저 글의 카테고리를 변경해주세요.`);
      return;
    }
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`text-xs px-3 py-1.5 rounded-full font-medium transition-colors ${
              showCategoryManager ? 'bg-orange-100 text-orange-700' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'
            }`}
          >
            🏷️ 카테고리
          </button>
      ...
      <main className="p-4 flex flex-col gap-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 text-center">
            ...
            <p className="text-2xl font-bold text-green-600">{loading ? '...' : posts.filter(p => p.published).length}</p>
          </div>
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 text-center">
            <p className="text-xs text-gray-400 mb-1">카테고리</p>
            <p className="text-2xl font-bold text-orange-500">{loading ? '...' : categories.length}</p>
          </div>
        </div>
              ...
              <div key={post.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400 mt-1">{new Date(post.created_at).toLocaleDateString('ko-KR')}</p>
                  </div>
                  <button
                    onClick={() => togglePublish(post)}
                    className={`text-xs px-2.5 py-1 rounded-full font-medium flex-shrink-0 ${post.published ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}
                  >
